...
class QccScraper:
    ...
    async def search_and_enter(self, company_name: str) -> bool:
        """搜索并进入详情页"""
        try:
    ...
    async def scrape_details(self, company_name: str) -> Dict:
        """[3/3] 提取详细信息"""
        data = {
    ...
    async def run(self, company_list: List[str]):
        ...
        for i, company in enumerate(company_list):
            ...
            if await self.search_and_enter(company):
                raw_data = await self.scrape_details(company)
                ...
                all_raw_data.append({'company_name': company, 'status': 'failed', 'error': '搜索失败'})
